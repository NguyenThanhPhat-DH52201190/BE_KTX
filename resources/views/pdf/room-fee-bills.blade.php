...
@section('title', 'Danh sách hóa đơn tiền phòng')
    ...
    <table class="pdf-table">
        <thead>
            <tr>
                ...
                <th class="text-right">Số tiền gốc</th>
        ...
        <tbody>
            @forelse ($bills as $index => $bill)
                <tr>
                    ...
                    <td class="text-center">{{ $bill['month'] }}/{{ $bill['year'] }}</td>
                    <td class="text-right">{{ \App\Support\VnFormat::currency($bill['original_amount'] ?? $bill['amount']) }}</td>
                    <td class="text-right">{{ $bill['discount_amount'] > 0 ? \App\Support\VnFormat::currency($bill['discount_amount']) : '—' }}</td>
