...
{
    protected const MAX_PER_IP = 10;
    protected const MAX_PER_CODE = 5;
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if (RateLimiter::tooManyAttempts($ipKey, static::MAX_PER_IP) || ($codeKey && RateLimiter::tooManyAttempts($codeKey, static::MAX_PER_CODE))) {
            return response()->json([
                'message' => static::TOO_MANY_ATTEMPTS_MESSAGE,
            ], 429);
        }

        RateLimiter::hit($ipKey, static::DECAY_SECONDS);
        if ($codeKey) {
            RateLimiter::hit($codeKey, static::DECAY_SECONDS);
        }
